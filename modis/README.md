...
|#  |Variable Name (keyword for `--variable`)|Description                           |Comments       |
|---|----------------------------------------|--------------------------------------|---------------|
...
> `MOD16A2GF.061`, `MYD16A2.061`, `MOD16A3GF.061` variables. This is a
> shortcoming of `gistool` and will be resolved in the future versions.


## Spatial and Temporal Extents
|#  |Variable Name (keyword for `--variable`)|Longitude Extents  |Latitude Extents  |Temporal Extents            |
|---|----------------------------------------|-------------------|------------------|----------------------------|
|1  |`MCD12Q1.006`                           |`-180`° to `+180`° |`-90`° to `+90`°  |`2001-01-01` to `2020-01-01`|
|2  |`MCD12Q1.061`                           |`-180`° to `+180`° |`-90`° to `+90`°  |`2001-01-01` to `2022-01-01`|
|3  |`MOD16A2.061`                           |`-180`° to `+180`° |`-90`° to `+90`°  |`2021-01-01` to `2024-02-10`|
|4  |`MOD16A2GF.061`                         |`-180`° to `+180`° |`-90`° to `+90`°  |`2000-01-01` to `2023-12-27`|
|5  |`MYD16A2.061`                           |`-180`° to `+180`° |`-90`° to `+90`°  |`2021-01-01` to `2024-02-18`|
|6  |`MOD16A3GF.061`                         |`-180`° to `+180`° |`-90`° to `+90`°  |`2000-02-18` to `2023-01-01`|

> [!NOTE]
> The spatial resolution of MODIS datasets is 500m globally.

# Other relevant information
## Land Cover Types
Below the land cover types for each values of the generated
`.tif` files by `gistool` is detailed based on [^1]. These
classes apply to the following variables of the MODIS datasets:
1. `MCD12Q1.006` and
2. `MCD12Q1.061`.

|Class Value|Name                               |Land Cover Description|
|-----------|-----------------------------------|----------------------|
...
## ET Values
Based on [^2], the ET values calculated from the following variables
...
[^1]: [https://lpdaac.usgs.gov/products/mcd12q1v061/](https://lpdaac.usgs.gov/products/mcd12q1v061/)
[^2]: [https://lpdaac.usgs.gov/products/mod16a3gfv061/](https://lpdaac.usgs.gov/products/mod16a3gfv061/)
